...
int main()
{
    ...
    setlocale(0, "");
    int K=4; // переменная для изначальной длинны массива
    ...
    cin >> S;
    K = K + S;// Задаем новую длинну массива
    A = (int*)realloc(A, K * sizeof(int));// Изменяем длину массива на заданную пользователем
    ...
    {
        cin >> A[i];
        if (A[i]==0 && !flag)// Если среди чисел есть 0 flag ставим на true
        {
            flag = true;
        }
    }
    if (flag)// Если среди чисел нашелся ноль то заменяем все четные числа на нули
    {
        for (int i = 0; i < K; i++)
            if (A[i] % 2 == 0)
            {
